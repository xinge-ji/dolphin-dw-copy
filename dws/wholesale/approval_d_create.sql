DROP TABLE IF EXISTS dws.wholesale_approval_d;
CREATE TABLE dws.wholesale_approval_d(
    -- 颗粒度
    ...
    -- 指标
    order_count int COMMENT '审批总单数',
    approved_order_count int COMMENT '审批通过单数',
    rejected_order_count int COMMENT '审批拒绝单数',
    approval_node_count int COMMENT '审批节点数',
    avg_node_approval_time int COMMENT '每个节点审批平均时长(分钟)'
)
...
-- 创建索引
CREATE INDEX IF NOT EXISTS idx_approval_d_entryid ON dws.wholesale_approval_d (entryid);
CREATE INDEX IF NOT EXISTS idx_approval_d_customid ON dws.wholesale_approval_d (customid);
CREATE INDEX IF NOT EXISTS idx_approval_d_stat_date ON dws.wholesale_approval_d (stat_date);